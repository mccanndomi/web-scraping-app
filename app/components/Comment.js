...
const Comment = ( props ) => {
    ...
    return(
        <View>
            <View style={[styles.container, {marginLeft: 25*indentSize}]}>
                <View style={[styles.topArea, { }]}>
                        <Avatar avatarName={comment.user} size={28}></Avatar>
                        <Text style={styles.userText}>Commented by {comment.user}</Text>
                        <Text style={styles.userText}>- {comment.date}</Text>
                </View>
                <View style={[styles.middleArea, { }]}>
                    {getCommentTitle(comment.title)}
                    {getCommentBody(comment.description)}
                </View>
            </View>
...
function getCommentTitle(title) {
  if (title == "" || title === "" || title == " "|| title === " ") {
    ...
    return <Text style={styles.title}>{title.replace("\n\n\n", "").replace("\n\n", "").replace("\n", "")}</Text>
  }
